...
public class MusicPoolManager : MonoBehaviour
{
    ...
    private Dictionary<string, AudioClip> clipCache;//用于本地缓存音效
    ...
    {
        if (!clipCache.TryGetValue(soundName, out AudioClip clip) || clip == null)
        {
            clip = ResourceManager.Instance.Load<AudioClip>(soundName);
